...
namespace DG.Tweening.Core
{
    public static class TweenManager
    {
        ...
        public static int maxActive = _DefaultMaxTweeners + _DefaultMaxSequences; // Always equal to maxTweeners + maxSequences
        ...
        public static Tween[] _activeTweens = new Tween[_DefaultMaxTweeners + _DefaultMaxSequences]; // public just to allow DOTweenInspector to access it
...
//            int toCompletedLoops = (int)((decimal)to / (decimal)t.duration); // Takes care of floating points imprecision (nahh doesn't work correctly either)
        ...
        static void RemoveActiveTween(Tween t)
        {
            ...
            if (index>=0 && index< _activeTweens.Length)
            {
                _activeTweens[index] = null;
            }
            else
            {
                //Debugger.LogRemoveActiveTweenError("RemoveActiveTween" + _activeTweens.Length+","+ index); 20181101 OYC
            }
