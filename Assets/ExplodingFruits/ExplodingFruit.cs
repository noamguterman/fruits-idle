...
public class ExplodingFruit : MonoBehaviour
{
	...
	#region fields
		...
		GameObject partsExplodedRoot;
		GameObject partsNonExplodedRoot;
	...
	void Awake()
	{
		...
		partsExplodedRoot = transform.Find("Parts exploded").gameObject;
		Transform partsNonExplodedRootTransform = transform.Find("Parts nonExploded");
		if ( partsNonExplodedRootTransform != null )
			partsNonExplodedRoot = partsNonExplodedRootTransform.gameObject;
		splashEffect = transform.Find("SplashEffect").gameObject;
		splashSounds = GetComponentsInChildren<AudioSource>();
		
		foreach ( Transform partTransform in partsExplodedRoot.transform )
		{
	...
	public void Explode ( Vector3? forceVector = null )
	{
		...
		//*** Split into parts
		{
			...
			partsExplodedRoot.SetActive( true );
			if ( partsNonExplodedRoot!= null )
				partsNonExplodedRoot.SetActive(false);
			...
			foreach ( Transform partTransform in origPositions.Keys )
			{
				...
				rigidbody.AddTorque( Vector3.Cross( partsExplodedRoot.transform.up, explodeDirection ));
			}
		}
	...
	public void Reset()
	{
		...
		if ( partsNonExplodedRoot != null )
			partsNonExplodedRoot.SetActive(true);
		
		partsExplodedRoot.SetActive( false );
