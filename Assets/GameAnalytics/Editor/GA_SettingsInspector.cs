...
{
    [CustomEditor(typeof(Setup.Settings))]
    public class GA_SettingsInspector : UnityEditor.Editor
    {
        ...
        void OnEnable()
        {
            Setup.Settings ga = target as Setup.Settings;
        ...
        public override void OnInspectorGUI()
        {
            ...
            GUILayout.Label("Unity SDK v." + Setup.Settings.VERSION);
            ...
            string updateStatus = GA_UpdateWindow.UpdateStatus(Setup.Settings.VERSION);
            ...
            if (ga.IntroScreen)
            {
                ...
                else
                {
                    ...
                    }))
                    {
                        ...
                        ga.SignUpOpen = false;
                        ga.CurrentInspectorState = Setup.Settings.InspectorStates.Account;
                    ...
                    {
                        ga.IntroScreen = false;
                        ga.CurrentInspectorState = Setup.Settings.InspectorStates.Basic;
                    }
                    EditorGUIUtility.AddCursorRect(GUILayoutUtility.GetLastRect(), MouseCursor.Link);
                    GUILayout.FlexibleSpace();
                    GUILayout.EndHorizontal();
            ...
            else
            {
                ...
                GUIStyle basicTabStyle = ga.CurrentInspectorState == Setup.Settings.InspectorStates.Basic ? activeTabStyleLeft : inactiveTabStyleLeft;

                if (ga.Studios == null)
                {
                    if (GUILayout.Button(_account, ga.CurrentInspectorState == Setup.Settings.InspectorStates.Account ? activeTabStyleLeft : inactiveTabStyleLeft))
                    {
                        ga.CurrentInspectorState = Setup.Settings.InspectorStates.Account;
                    }

                    basicTabStyle = ga.CurrentInspectorState == Setup.Settings.InspectorStates.Basic ? activeTabStyle : inactiveTabStyle;
                }
                ...
                if (GUILayout.Button(_advanced, ga.CurrentInspectorState == Setup.Settings.InspectorStates.Pref ? activeTabStyleRight : inactiveTabStyleRight))
                {
                    ga.CurrentInspectorState = Setup.Settings.InspectorStates.Pref;
                }

                GUILayout.EndHorizontal();

                #region Settings.InspectorStates.Account
                if (ga.CurrentInspectorState == Setup.Settings.InspectorStates.Account)
                {
                    EditorGUILayout.Space();
                ...
                else if (ga.CurrentInspectorState == Setup.Settings.InspectorStates.Basic)
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.Space();
                ...
                else if (ga.CurrentInspectorState == Setup.Settings.InspectorStates.Pref)
                {
                    EditorGUILayout.Space();
                    EditorGUILayout.Space();
        ...
        private MessageType ConvertMessageType(Setup.Settings.MessageTypes msgType)
        {
            switch (msgType)
            {
                case Setup.Settings.MessageTypes.Error:
                    return MessageType.Error;
                case Setup.Settings.MessageTypes.Info:
                    return MessageType.Info;
                case Setup.Settings.MessageTypes.Warning:
        ...
        public static void SignupUser(Setup.Settings ga, GA_SignUp signup)
        {
...
#if UNITY_2018_3_OR_NEWER
        private static IEnumerator SignupUserFrontend(UnityWebRequest www, Setup.Settings ga, GA_SignUp signup)
#else
        ...
        private static void LoginUser(Setup.Settings ga)
        {
            Hashtable jsonTable = new Hashtable();
            jsonTable["email"] = ga.EmailGA;
            jsonTable["password"] = ga.PasswordGA;
...
#if UNITY_2018_3_OR_NEWER
        private static IEnumerator LoginUserFrontend(UnityWebRequest www, Setup.Settings ga)
#else
        ...
        private static void GetUserData(Setup.Settings ga)
        {
#if UNITY_2018_3_OR_NEWER
...
#if UNITY_2018_3_OR_NEWER
        private static IEnumerator GetUserDataFrontend(UnityWebRequest www, Setup.Settings ga)
#else
...
#endif
                {
                    ...
                    else if (returnParam != null)
                    {
                        ...
                        for (int s = 0; s < studioList.Count; s++)
                        {
                            ...
                            {
                                List<Setup.Game> returnGames = new List<Setup.Game>();
                                ...
                                for (int g = 0; g < gamesList.Count; g++)
                                {
                                    ...
                                    returnGames.Add(new Setup.Game(games["name"].ToString(), int.Parse(games["id"].ToString()), games["key"].ToString(), games["secret"].ToString()));
                                }
        ...
        public static void CreateGame(Setup.Settings ga, GA_SignUp signup, int studioIndex, string gameTitle, string googlePlayPublicKey, RuntimePlatform platform, AppFiguresGame appFiguresGame)
        {
            Hashtable jsonTable = new Hashtable();
...
#if UNITY_2018_3_OR_NEWER
        private static IEnumerator CreateGameFrontend(UnityWebRequest www, Setup.Settings ga, GA_SignUp signup, RuntimePlatform platform, AppFiguresGame appFiguresGame)
#else
        ...
        public static void GetAppFigures(Setup.Settings ga, GA_SignUp signup)
        {
#if UNITY_2018_3_OR_NEWER
...
#if UNITY_2018_3_OR_NEWER
        private static IEnumerator GetAppFiguresFrontend(UnityWebRequest www, Setup.Settings ga, GA_SignUp signup, string gameName)
#else
...
#endif
                    {
                        Selection.objects = new UnityEngine.Object[] { AssetDatabase.LoadAssetAtPath("Assets/Resources/GameAnalytics/Settings.asset", typeof(Setup.Settings)) };
                        ga.CurrentInspectorState = Setup.Settings.InspectorStates.Account;
        ...
        private static void SelectStudio(int index, Setup.Settings ga, int platform)
        {
        ...
        private static void SelectGame(int index, Setup.Settings ga, int platform)
        {
        ...
        private static void SetLoginStatus(string status, Setup.Settings ga)
        {
        ...
        {
            if (Setup.Settings.CheckingForUpdates)
            {
                return;
            }

            Setup.Settings.CheckingForUpdates = true;
#if UNITY_2018_3_OR_NEWER
...
#endif
        {
...
#endif
                {
                    ...
                    if (returnParam.ContainsKey("unity"))
                    {
                        ...
                        if (unityParam.ContainsKey("version"))
                        {
                            ...
                            if (IsNewVersion(newVersion, Setup.Settings.VERSION))
                            {
            ...
            {
                Setup.Settings.CheckingForUpdates = false;
            }
        }
            ...
            try
            {
...
#endif
                {
                    ...
                    for (int i = 0; i < unity.Count; i++)
                    {
                        ...
                        if (unityHash["version"].ToString() == Setup.Settings.VERSION)
                        {
                            break;
                        }
