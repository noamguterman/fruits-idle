...
namespace Coffee.UIExtensions
{
...
#endif
	{
        ...
        public int screenWidth
        {
            get
            {
                ...
                if (!Application.isPlaying)
                    return int.Parse(UnityEditor.UnityStats.screenRes.Split('x')[0]);
#endif
                return Screen.width;
            }
        }
...
#if UNITY_EDITOR
                if (!Application.isPlaying)
                    return int.Parse(UnityEditor.UnityStats.screenRes.Split('x')[1]);
#endif
		...
		void SetupCommandBuffer()
		{
            ...
            var rtId = new RenderTargetIdentifier(capturedTexture);
            ...
            _buffer.GetTemporaryRT(s_CopyId, screenWidth, screenHeight, 0, m_FilterMode);
#if UNITY_EDITOR
            if (!Application.isPlaying)
            {
                _buffer.Blit(Resources.FindObjectsOfTypeAll<RenderTexture>().FirstOrDefault(x => x.name == "GameView RT"), s_CopyId);
            }
            else
#endif
            {
                _buffer.Blit(BuiltinRenderTextureType.CameraTarget, s_CopyId);
            }
